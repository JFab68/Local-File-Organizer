...
if (-not $SkipEnvCheck) {
    ...
    if (Test-CommandExists "python") {
        ...
        Write-ColorText "✗ Python not found. Please install Python 3.8+ first." $Red
        exit 1
    }
    ...
    if ($condaFound) {
        ...
        Write-ColorText "✓ Found: $condaVersion" $Green
    }
    else {
        Write-ColorText "✗ Conda not found. Installing Miniconda..." $Yellow
        ...
        if (Test-Path $condaCommand) {
            Write-ColorText "✓ Miniconda installed successfully" $Green
        }
        else {
            Write-ColorText "✗ Failed to install Miniconda" $Red
            exit 1
        }
    }
    ...
    if (Test-CommandExists "tesseract") {
        ...
        Write-ColorText "✓ Found: $tesseractVersion" $Green
    }
    else {
        Write-ColorText "⚠ Tesseract OCR not found in PATH. Please ensure it's installed and added to PATH." $Yellow
...
try {
    ...
    Write-ColorText "✓ Environment created successfully" $Green
}
catch {
    if ($_.Exception.Message -like "*already exists*") {
        ...
        Write-ColorText "✗ Failed to create environment: $($_.Exception.Message)" $Red
        exit 1
    }
}
...
$nexaInstallCmd = @"
call "$activateScript" local_file_organizer && pip install nexaai --prefer-binary --index-url https://nexaai.github.io/nexa-sdk/whl/cpu --extra-index-url https://pypi.org/simple --no-cache-dir
"@
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-ColorText "✗ Failed to install Nexa SDK" $Red
    exit 1
}
...
$requirementsCmd = @"
call "$activateScript" local_file_organizer && pip install -r requirements.txt
"@
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-ColorText "⚠ Some requirements may have failed. Installing individually..." $Yellow
    ...
    foreach ($package in $packages) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-ColorText "✓ $package installed" $Green
        }
        else {
            Write-ColorText "✗ Failed to install $package" $Red
        }
    }
}
...
$nltkCmd = @"
call "$activateScript" local_file_organizer && python -c "import nltk; nltk.download('stopwords', quiet=True); nltk.download('punkt', quiet=True); nltk.download('wordnet', quiet=True); print('NLTK data downloaded successfully')"
"@

cmd /c $nltkCmd
if ($LASTEXITCODE -eq 0) {
    Write-ColorText "✓ NLTK data downloaded successfully" $Green
}
else {
    Write-ColorText "⚠ NLTK data download may have issues" $Yellow
}
...
$diagnosticCmd = @"
call "$activateScript" local_file_organizer && python diagnostic_simple.py
"@

cmd /c $diagnosticCmd
if ($LASTEXITCODE -eq 0) {
    Write-ColorText "✓ Diagnostic test passed" $Green
}
else {
    Write-ColorText "⚠ Diagnostic test had issues - check output above" $Yellow
}
...
Write-ColorText "✓ Batch files created successfully" $Green
Write-Host ""
